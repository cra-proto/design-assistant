...
export interface BookmarkletItem {
  buildHref: (origin: string) => string;
  titleKey: string;
...
export const BOOKMARKLETS = {
  aida: {
    buildHref: (origin) =>
      `javascript:(function(){const url=encodeURIComponent(window.location.href);const title=encodeURIComponent(document.title);window.open('${origin}/import-page?org=CRA&url='+url+'&title='+title,'_blank');})();`,
...
export class BookmarkletsComponent {
  ...
  protected readonly items = BOOKMARKLETS;
  public readonly keys = input.required<BookmarkletKey[]>();

  private readonly hrefs: Record<BookmarkletKey, SafeUrl> = Object.fromEntries(
    (Object.keys(BOOKMARKLETS) as BookmarkletKey[]).map((key) => [key, this.sanitizer.bypassSecurityTrustUrl(BOOKMARKLETS[key].buildHref(window.location.origin))]),
  ) as Record<BookmarkletKey, SafeUrl>;

  protected getHref(key: BookmarkletKey): SafeUrl {
