...
import { AddPagesState, UrlItem, ValidationState, BreadcrumbValidationState } from '../add-pages.model';
import { BreadcrumbNode } from '../../ia-assistant/data/data.model';
...
export class AddPagesStateService {
    ...
    get urlsChecking(): UrlItem[] {
        return this.validationState().urls.filter(u => u.status === 'checking');
    }

    get urlsBlocked(): UrlItem[] {
        return this.validationState().urls.filter(u => u.status === 'blocked');
    }

    get urlsBad(): UrlItem[] {
        return this.validationState().urls.filter(u => u.status === 'bad');
    }

    get urlsRedirected(): UrlItem[] {
        return this.validationState().urls.filter(u => u.status === 'redirect');
    }

    get urlsOk(): UrlItem[] {
        return this.validationState().urls.filter(u => u.status === 'ok');
    }

    // Reset validation state
    resetValidation() {
        this.validationState.set({
            rawUrls: this.validationState().rawUrls, // preserve the textarea input
            urls: [],
            urlTotal: 0,
            urlChecked: 0,
            urlPercent: 0,
